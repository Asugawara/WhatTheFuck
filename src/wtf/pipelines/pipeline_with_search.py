...
class CommandOutputAnalyzerWithSearch(PipelineBase):
    def __init__(
        ...
        available_netlocs: tuple[str, ...] | None = None,
    ) -> None:
    ...
    def _build_pipeline(self):  # type: ignore
        ...
        pipe.add_component(
            ...
            self._factory_generator(
                self._model, self._openai_api_key, self._anthropic_api_key, None
            ),
        )
        pipe.add_component("llm_output_adapter", OutputAdapter(template="{{ replies[0] }}", output_type=str))
        pipe.add_component("search_google", WebSearch(self._available_netlocs))
        pipe.add_component("link_content_fetcher", LinkContentFetcher())
        ...
        pipe.connect("search_google", "link_content_fetcher")
